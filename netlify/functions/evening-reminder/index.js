...
exports.handler = async () => {
    ...
    structuredQuery: {
      ...
      where: {
        compositeFilter: {
          op: 'AND',
          filters: [
            { fieldFilter: { field: { fieldPath: 'mealTime' },   op: 'EQUAL',                value: { stringValue:   'evening'    } } },
  ...
  try {
    ...
    if (wasFed) {
      console.log('Evening feed already done — no reminder needed.');
  ...
  const payload = JSON.stringify({
    title: `${dogName} needs dinner! 🍽️`,
    body:  `${dogName} hasn't been fed yet this evening — who's on it?`,
    url:   `${APP_URL}/#feed`,
  });
  ...
  console.log(`Evening reminder sent to ${sends.length} device(s).`);
